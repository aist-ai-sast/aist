...
@login_required
@require_POST
def gerrit_projects_list(request: HttpRequest) -> JsonResponse:
    """
    Return a lightweight list of projects from a Gerrit instance.

    Requires ``organization_id``: resolves the default active Gerrit integration
    for that organization and uses its stored credentials.
    """
    organization_id = (request.POST.get("organization_id") or "").strip()
    if not organization_id:
    ...
    integration = (
        OrgIntegration.objects
        .filter(organization_id=organization_id, integration_type="GERRIT", is_active=True)
        .order_by("pk")
        .first()
    )
    if not integration:
        return JsonResponse(
            {"ok": False, "error": "No active Gerrit integration found for this organization."},
            status=404,
        )
