...
| An external provider describes its own run — what it covered, what it spent — alongside the findings | Reported values are bounded in type, size, and character content before they are stored, and a description the platform does not recognize is read past without being acted on, while run identity, provider selection, and report structure stay closed to unrecognized keys | Coverage and cost figures are the provider's claims about its own work; AIST surfaces a breakdown that contradicts its own total but cannot independently verify what a run actually examined |
