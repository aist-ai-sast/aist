...
| A scan or connector attempts to affect the application host | Workloads run in operation-specific containers without privileged or host-network execution | Workers with Docker-daemon access remain privileged host components and require isolated, hardened hosts |
